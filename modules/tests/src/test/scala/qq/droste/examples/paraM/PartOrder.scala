package qq.droste.examples.paraM
...
import qq.droste.{RAlgebraM, scheme}

final class PartOrder extends Properties("PartOrder") {

  import PartOrder._

  def insertTuple(x: (Int, Int), l: List[(Int, Int)]): Option[List[(Int, Int)]] =
  ...
  property("non-empty insert") =
    insertTuple((2, 2),
      List((0, 0), (1, 1), (3, 3))) ?= Some(List((0, 0), (1, 1), (2, 2), (3, 3)))

  property("failing insert") =
    insertTuple((1, 1),
      List((0, 0), (0, 2), (2, 2))) ?= None
...
object PartOrder {
  ...
  def insertM[M[_], A](cmp: (A, A) => M[Boolean], x: A)(implicit M: Monad[M]): List[A] => M[List[A]] =
    scheme.zoo.paraM(
      RAlgebraM[List[A], M, ListF[A, ?], List[A]] {
        case NilF =>
          M.pure(List(x))
        case ConsF(h, (l, rec)) =>
          cmp(x, h).flatMap(b => M.pure(if (b) x :: h :: l else h :: rec))
      }
    )

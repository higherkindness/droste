...
package object data {
  ...
  /** A fix point function for types.
    *
    * Implemented as an obscured alias: {{{type Fix[F[_]] = F[Fix[F]]}}}
    *
    * The companion object can be used to translate between representations.
    */
  type Fix[F[_]] // = F[Fix[F]]

  /** A fix point function for types that adds an additional attribute to each
    * node in the resulting data structure.
    *
  ...
  /** The pattern functor for [[Attr]].
    ...
    *
    * Implemented as an obscured alias: {{{type AttrF[F[_], A, B] = (A, F[B])}}}
    *
    * The companion object can be used to translate between representations.
    */
  type AttrF[F[_], A, B] // = (A, F[B])

  /** A fix point function for types that allows for the replacements of nodes
    * in the data structure with values of a different type.
    *

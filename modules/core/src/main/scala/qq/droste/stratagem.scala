...
object stratagem {
  ...
  def attributeCata[F[_]: Functor, A](
      algebra: Algebra[F, A]): Fix[F] => Attr[F, A] =
    scheme.cata(
      Trans((fa: F[Attr[F, A]]) => AttrF(algebra(fa.map(_.head)), fa)).algebra)

  /** An algebra for listing all possible partial structures */
  def allPartials[F[_]: Traverse, A]: Algebra[
    AttrF[F, A, ?],
    List[Coattr[F, A]]] =
  ...
  /** An algebra for listing partial structures showing the path a fold might
    ...
    */
  def perimeterPartials[F[_]: Traverse, A]: Algebra[
    AttrF[F, A, ?],
    List[Coattr[F, A]]] =

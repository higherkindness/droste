...
object prelude {
  ...
  implicit def cofreeToRefTree[F[_] <: AnyRef: Traverse, A](implicit
  ...
  implicit def freeToRefTree[F[_] <: AnyRef: Traverse, A](implicit
      evF: ToRefTree[F[RefTree]],
      evA: ToRefTree[A]
  ...
  private def fixedToRefTreeAlgebra[F[_]](implicit
      evF: ToRefTree[F[RefTree]]
  ): Algebra[F, RefTree] =
    Algebra((fa: F[RefTree]) => evF.refTree(fa))

  private def cofreeToRefTreeAlgebra[F[_] <: AnyRef, A](implicit
      evF: ToRefTree[F[RefTree]],
      evA: ToRefTree[A]
  ...
  private def freeToRefTreeAlgebra[F[_] <: AnyRef, A](implicit
      evF: ToRefTree[F[RefTree]],
      evA: ToRefTree[A]

...
class Checks extends Properties("athema") {
  ...
  lazy val fdPairs: List[(Expr.Fixed[Double], Expr.Fixed[Double])] = List(
    ...
    Prod(vary, vary) -> Add(Prod(vary, Const.fix(0.0)), Prod(Const.fix(0.0), vary))
  )
  ...
  property("∇f") = {
    ...
    lazy val exprs: List[(Ex, IJK)] = List(
      Prod(varx, varx) -> IJK(Prod(Const.fix(2.0), varx), Const.fix(0.0), Const.fix(0.0)),
      Prod(varx, vary) -> IJK(vary, varx, Const.fix(0.0)),
      Prod(vary, vary) -> IJK(Const.fix(0.0), Prod(Const.fix(2.0), vary), Const.fix(0.0)),
      Prod(Prod(varx, vary), varz) -> IJK(Prod(vary, varz), Prod(varx, varz), Prod(varx, vary)))
